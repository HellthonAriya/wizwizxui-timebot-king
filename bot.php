...
if(preg_match('/payWithWeSwap(.*)/',$data,$match)) {
    ...
    if(isset($pay->pay_address)){
        ...
        sendMessage("لطفا مبلغ " . $priceInTrx . " ترون توسط لینک زیر پرداخت کنید(پرداخت شما به ریال هست!)، بعد از پرداخت حدود 1 الی 15 دقیقه صبر کنید تا پرداخت به صورت کامل انجام شود سپس روی پرداخت کردم کلیک کنید ",$keys);
    }else{
...
if($data == 'reciveApplications') {
    ...
    $keyboard = array_chunk($keyboard,1);
    editText($message_id, "
🔅شما میتوانید برنامه های پشتیبانی شده کانفیگ های ما رو ازین جا دریافت کنید
❗️نکات مهمی درباره سرویس ما هست که حتما در چنل، آموزش ها و توضیحات رو مطالعه کنید
